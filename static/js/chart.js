...
(function () {
  ...
  function loadResultChart(ticker, periodFrom, periodTo) {
    ...
    // ⚠️ restoreDrawState({}) 는 매칭 점선 데이터(matchPoints) 까지 지우므로
    //    사용자 도형만 비우는 clearUserDrawingsOnly 사용.
    if (typeof window.clearUserDrawingsOnly === 'function') {
      window.clearUserDrawingsOnly();
    }
