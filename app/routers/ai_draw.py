"""
AI 라우터 (Pro 전용) — Gemini 기반.

엔드포인트:
  POST /api/ai/analyze_drawing       — 유저가 그린 그림을 AI가 보정 + 질문 (메인)
  POST /api/ai/pattern_from_text     — 자연어 → 패턴 생성
  POST /api/ai/extract_from_image    — 이미지 → 패턴 추출 (UI 숨김)

AI API 는 위 세 경로에서만 호출됩니다. 다른 곳에서 자동 호출되지 않음.
"""
import logging
import time
...
from app.services import ai_input_guard, gemini_service
