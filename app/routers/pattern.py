...
async def pattern_search(body: PatternSearchRequest, user: dict = Depends(require_user)):
    ...
    if market == "US":
    ...
    elif market in ("CRYPTO_KRW", "CRYPTO_USDT"):
        # 크립토는 365일/년 (주식 영업일 ~252 와 다름)
        from app.services import crypto_data_service
        if not crypto_data_service.is_built(market):
            crypto_data_service.build_crypto_cache(years=2)
        ohlcv_cache = crypto_data_service.all_crypto_ohlcv(market=market)
        names_cache = crypto_data_service.all_crypto_names(market=market)
        ...
        logger.info("%s 검색: lookback=%d, 페어 %d개",
                    market, effective_lookback, len(ohlcv_cache))
    elif tf == "daily":
